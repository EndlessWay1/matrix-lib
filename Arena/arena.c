...
void arena_temp_end(mem_arena_temp temp_arena){
    arena_pop_to(temp_arena.arena, temp_arena.start_pos);
}
...
mem_arena_temp arena_scratch_get(mem_arena** conflicts, u32 num_conflicts){
    ...
    mem_arena** selected = &_scratch_arena[sractch_index];
...
b32 plat_mem_commit(void* ptr, u64 size){
    void* ret = VirtualAlloc(ptr, size, MEM_COMMIT, PAGEREADWRITE);
    return ret != NULL;
}
